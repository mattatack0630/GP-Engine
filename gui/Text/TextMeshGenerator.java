...
public class TextMeshGenerator
{
	...
	public static VaoObject generateTextVao(String text, TextAttributes attribs, Font font, VaoObject VAO)
	{
		...
		// Split text into wrapped lines, so that they are split per word instead of per character
		...
		{
			vaoObj.update(positionsArray, VaoObject.POSITIONS);
			vaoObj.update(textureCoordsArray, VaoObject.TEXTURE_COORDS);
			vaoObj.vertexCount = positionsArray.length / 2;
		}
