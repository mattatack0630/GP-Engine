...
public class Sorter
{
    ...
    public static <E> List<E> dynamicSort(List<E> start, E element, Comparator<E> comparator)
    {
        int predIndex = start.isEmpty() ? 0 : binaryFindPred(start, 0, start.size() - 1, element, comparator);
        start.add(predIndex, element);
        return start;
    }

    public static <E extends Comparable> int binaryFindPred(List<E> list, E findEl)
    {
        if (list.size() > 0)
            return binaryFindPred(list, 0, list.size() - 1, findEl);
        return 0;
    }

    private static <E extends Comparable> int binaryFindPred(List<E> list, int start, int end, E findEl)
    {
    ...
    private static <E> int binaryFindPred(List<E> list, int start, int end, E findEl, Comparator<E> comparator)
    {
        // ordered least -> greatest
        int midIndex = (start + end) / 2;
        E elMid = list.get(midIndex); // check access speed
        int c = comparator.compare(elMid, findEl);

        if (start >= end)
            return (c == -1) ? midIndex + 1 : midIndex;
        if (c == -1)
            return binaryFindPred(list, midIndex + 1, end, findEl, comparator);
        if (c == 1)
            return binaryFindPred(list, start, midIndex - 1, findEl, comparator);
